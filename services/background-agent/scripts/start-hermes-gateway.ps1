...
$envFile = Join-Path $PSScriptRoot "..\background-agent.env"; $envEx = Join-Path $PSScriptRoot "..\background-agent.env.example"; if (-not (Test-Path $envFile) -and (Test-Path $envEx)) { $envFile = $envEx }
