...
(function () {
  ...
  async function loadHealth() {
    ...
    try {
      ...
      ["main_llm", "summarizer", "embedding", "memory_store", "tts"].forEach(function (p) {
        setStatusDot(p, false, String(e.message || e).slice(0, 60));
      });
    }
  }
  ...
  async function loadNetwork() {
    try {
      ...
      if (st) st.textContent = "Failed to load network settings: " + e.message;
    }
  }
  ...
  async function saveNetwork() {
    ...
    try {
      const data = await apiJson("PUT", "/v1/settings/network", payload);
  ...
  async function loadNamespaces() {
    ...
    try {
      ...
      list.innerHTML = '<div class="input-hint" style="color: var(--warning-color);">Failed to load: ' + (e.message || e) + "</div>";
    }
  }
  ...
  async function syncWiki() {
    ...
    try {
      ...
      if (st) st.textContent = "Sync failed: " + e.message;
    }
  }
  ...
  async function pasteWiki() {
    ...
    try {
      ...
      if (st) st.textContent = "Ingest failed: " + e.message;
    }
  }
