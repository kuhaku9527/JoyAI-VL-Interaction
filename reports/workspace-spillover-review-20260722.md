...
| 位置 | 内容 | 体量 | 判定 | 泄漏来源 |
|---|---|---|---|---|
| `D:/c/Users/22186/.workbuddy` | `binaries/python`（managed runtime 骨架） | ~36K | WorkBuddy HOME 错配 | 某会话 HOME 被解析成 `D:/c/Users/22186` |
...
**重要排除项**：未发现任何个人文件（无 `Documents`/`Desktop`/`Pictures`/照片/其他项目源码）。`D:/c/Users/22186/.workbuddy` 只是错位的工作目录，不是真实用户档案。`D:/d/AI/workspace/...` 是空壳（无 `.git`、仅 2 个顶层条目）。**全部判定为 Joy/WorkBuddy 相关，可安全清理。**

## 2. 外溢根因（4 条泄漏通道）

1. **HOME 错配**：WorkBuddy 在某会话中 `HOME` 被解析到 `D:/c/Users/22186`，managed python runtime 落到那里。
...
- `.cache/` 加入 `.gitignore`。
- 确认 WorkBuddy `HOME` 回到 `C:/Users/22186/.workbuddy`（删掉 `D:/c` 那份错位 HOME）。
...
- **P4 错位 HOME（`D:/c`）**：`D:/c/Users/22186/.workbuddy` → 确认真实 HOME 正常后删 `D:/c` 整棵。
