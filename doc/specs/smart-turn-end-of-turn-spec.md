...
- **风险**：Smart Turn 在思考停顿时可能误判「说完」（0.1% CPU，~8.6MB 模型 / CPU int8 ONNX，+50ms 延迟），需 e2e 验证。

## §2 范围与负面约束（What NOT）
- **做**：`services/webui/src/joy_interaction_webui/smart_turn_adapter.py` 用 `onnxruntime` 直接加载 **pipecat-ai/smart-turn-v3 的 `smart-turn-v3.2-cpu.onnx`（~8.6MB，CPU int8）**（**不依赖 `pipecat` 包**，仓库未安装）；在**编排器结束判定**（`jarvis_mode.py` ASR endpoint → 调 LLM 分支，原 L989 附近）接入，返回 complete+probability 决定是否跳出 DIALOG_ACTIVE 走 LLM。
...
- 模型：`smart-turn-v3.2-cpu.onnx`（~8.6MB，CPU int8，**audio-native：仅吃 16kHz 音频、忽略 transcript 文本**），路径 `SMART_TURN_MODEL_PATH` 或 `<JOYAI_MODELS_ROOT>/smart-turn/`；已从 HuggingFace（`pipecat-ai/smart-turn-v3`）拉取至 `D:/AI/models/smart-turn/`。
...
4. **硬依赖（已闭环，2026-08-07）**：ONNX 模型已从 `pipecat-ai/smart-turn-v3` 拉取 `smart-turn-v3.2-cpu.onnx`（~8.6MB int8）至 `D:/AI/models/smart-turn/`，并 vendored `openai/whisper-small` 的 `preprocessor_config.json`（~185KB）。适配器现已按**验证过的契约**实现：`input_features [1,80,800]`（Whisper log-mel）+ `outputs[0][0]` 已是 sigmoid 概率、`>0.5` 判说完；模型 audio-native，忽略 transcript。接线默认关闭（`SMART_TURN_ENABLED`），fail-open 保证默认行为零变化。剩余：用户 venv `pip install onnxruntime transformers` 后跑 pytest 验真实推理 + 阈值 e2e 标定（需真实录音语料）。
