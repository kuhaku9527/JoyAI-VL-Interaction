...
- **#2 MiniMax Token Plan 接入**（2026-07-12 半落地）：声音克隆走云端 `speech-2.8-hd`，voice_id `minimax_man_33333` 已建（MiniMax Token Plan 凭证 + GroupId `2073018417979330937`），BT-7274 persona 链路测试通过（`vllm_inference=948ms`，prompt_tokens=511 含 character_profile）。**未消除**：`run-windows.env` 凭证沉淀 + 全链路 e2e 收尾待办。详见 `doc/voice-clone.md` §13。
