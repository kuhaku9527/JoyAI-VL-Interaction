...
不需要任务管理器。仓库根目录有 `stop-joyai.ps1`，一键干 12 个服务（llama / voice-clone / webui / tts-adapter 等）：

```powershell
# 默认：杀掉全部 11+ 服务（PID 文件 + 端口监听双管齐下）
...
🚑 If you run into deployment issues, see the [Troubleshooting Guide](doc/deprecated/troubleshooting.md).
...
📐 For the full architecture diagram and component details, see the [Architecture Guide](doc/deprecated/architecture.md).
